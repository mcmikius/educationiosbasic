...
protocol Team{
    ...
    associatedtype P : Player // тип должен быть реализацией протокола Player
    
    var maximumPlayersCount: UInt {get}
    
    var players:[UInt:P?] {get set}
    ...
    func showPlayersSalary(amount: UInt) -> [UInt: P?]
...
extension Team{
    ...
    var count:Int {
        return players.filter { $1 != nil }.count
    }
    
    func showPlayersSalary(amount: UInt) -> [UInt: P?] { // здест также меняем тип на generics
...
class FootballTeam : Team{
    ...
    //    typealias P = BaseFootballPlayer // можно указать тип явно (либо оставить это компилятору)
    
    var name: String
    
    var maximumPlayersCount:UInt = 11
    
    var players:[UInt:BaseFootballPlayer?] = [UInt:BaseFootballPlayer?]() // отсюда компилятор узнает тип P
    ...
    func playMatch(){
        players.forEach(){
            $1?.move()
            $1?.kickTheBall() // теперь метод доступен, т.к. мы через generics передали тип BaseFootballPlayer, где находится этот метод
        }
    }
